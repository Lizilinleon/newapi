...
func countUsersByEmail(tx *gorm.DB, email string) (int64, error) {
	...
	var count int64
	err := emailQuery(tx, email).Count(&count).Error
	return count, err
}
...
func GetAllUsers(pageInfo *common.PageInfo, viewerRole int) (users []*User, total int64, err error) {
	// Start transaction
	tx := DB.Begin()
	if tx.Error != nil {
	...
	if viewerRole != common.RoleRootUser {
		query = query.Where("role < ?", viewerRole)
	}
...
func SearchUsers(keyword string, group string, role *int, status *int, startIdx int, num int, viewerRole int) ([]*User, int64, error) {
	var users []*User
	var total int64
	var err error
